...
use crate::{
    app::{Focus, GraphAction},
    theme::Theme,
    widget::{ConfigKeyResult, ConnectionPanel, LoadingState, Toast, handle_config_key},
};
...
impl GraphView {
    ...
    fn draw_parsed_data_chart(
        ...
        let Some(graph) = buffer.graph() else {
            ...
            Paragraph::new(msg).style(Theme::muted()).render(area, buf);
            return;
        };
        ...
        self.cached_graph_data
            .resize(visible_series.len(), Vec::new());
    ...
    fn draw_packet_rate_chart(
        ...
        // Calculate bounds
        let all_x: Vec<f64> = self.cached_graph_data[0]
            .iter()
            .chain(self.cached_graph_data[1].iter())
            .map(|(x, _)| *x)
            .collect();
        let all_y: Vec<f64> = self.cached_graph_data[0]
            .iter()
            .chain(self.cached_graph_data[1].iter())
            .map(|(_, y)| *y)
            .collect();
    ...
    fn draw_config(
        ...
        let settings_focused =
            focus == Focus::Config && self.config_sub_focus == ConfigSubFocus::Settings;
    ...
    fn draw_series_section(
        ...
        for (i, (name, series)) in graph.series.iter().enumerate() {
            ...
            let max_name_len = (inner.width as usize)
                .saturating_sub(prefix.len() + visibility.len() + latest.len() + 2);
    ...
    fn handle_config_key(&mut self, key: KeyEvent, handle: &SessionHandle) -> Option<GraphAction> {
        ...
        if key.code == KeyCode::Tab && has_series && !self.config_nav.edit_mode.is_dropdown() {
            if was_text_editing {
                ...
                let _ = self
                    .config_nav
                    .apply_text_edit(GRAPH_CONFIG_SECTIONS, &mut self.config);

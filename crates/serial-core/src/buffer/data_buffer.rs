//! DataBuffer - central data management for serial monitor
//!
//! The [`DataBuffer`] is the main orchestrator handling:
//! - Raw data storage
//! - Encoding to display strings
//! - Filtering by pattern and direction
//! - Search within filtered view
//! - Size-based truncation
//! - Graph engine integration (lazy)

use std::collections::VecDeque;
use std::time::SystemTime;

use super::chunk::{ChunkView, Direction, RawChunk};
...
impl DataBuffer {
    ...
    /// Get raw chunks (for file saving, etc.)
    pub(crate) fn raw_chunks(&self) -> &VecDeque<RawChunk> {
        &self.raw_chunks
    }
}
...
#[cfg(test)]
mod tests {
    // Integration tests will be added once all components are complete
}
